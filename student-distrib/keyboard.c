...
void init_keyboard(){
    ...
    keyboardFlags[curr_rid].piano = 0;
...
extern uint8_t shortcut_handler(uint8_t scancode){
    ...
    if(keyboardFlags[curr_kid].alt == 1){
        ...
        switch (scancode)
        {
        case 0x3B:  // F1: Switch to T1
            switch_handler(0);
            return 1;     
            //terminal_printf("received switch to 0");
            break;
        case 0x3C:  // F1: Switch to T1
            switch_handler(1);
            return 1;     
            //terminal_printf("received switch to 1");
            break;
        case 0x3D:  // F1: Switch to T1
            switch_handler(2);
            return 1;     
        ...
        case 0x25:
            if(keyboardFlags[curr_kid].piano == 0){
                keyboardFlags[curr_kid].piano = 1;
                terminal_printf("START YOUR PERFORMANCE BRUHHHHHHH!");
            }else{
                keyboardFlags[curr_kid].piano = 0;
                for(i = 0; i < 34; i++){                    
                    terminal_backspace();
                }
            }
...
extern void switch_handler(uint8_t target_kid){
    ...
    if(keyboardFlags[curr_kid].piano == 1) return;
    mem_switch(curr_tid, target_kid);
    //  switch current display terminal
    update_video_page_switch(target_kid, curr_tid);
    curr_tid = target_kid;
    curr_kid = target_kid;
...
extern void piano_handler(uint8_t scancode){
    cli();
    if(scancode > 0x58){
        nosound();
        return;
...
extern void keyboard_handler(){
    ...
    if(keyboardFlags[curr_kid].piano){
        piano_handler(scancode);
        send_eoi(KEYBOARD_IRQ);
        sti();
        return;
    }
