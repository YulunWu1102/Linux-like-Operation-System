...
/* void mem_switch(uint8_t prev_kid, uint8_t new_kid);
 * Inputs: prev_kid, the previoud keyboard/viewing index
 * Return: new_kid, the one we switching to
 * Function: do two memcopy for video memory buffers */
void mem_switch(uint8_t prev_kid, uint8_t new_kid){
    copy_video_page(prev_kid+1, 0);
    copy_video_page(0, new_kid+1);
 ...
 * Inputs: -
 * Return: the position of memory buffer that curr_rid is working on
 * Function: the position of memory buffer that curr_rid is working on */
uint8_t* get_current_write_addr(){
